...
public class ManageSystem extends JFrame{ //JFrame클래스는 최상위 컨테이너 역할의 클래스이다. 
	...
	class MenuMain extends JPanel implements ActionListener,ItemListener{
		...
		FileDialog readOpen, saveOpen;					      // 전역 객체 참조변수 선언
		...
		public MenuMain() {//화면 구현(UI), 각 객체(컴퍼넌트)에 대한 이벤트 연결
			...
			//파일 메뉴 이벤트 연결
			fopen.addActionListener(this);  //'열기' this는 이벤트 핸들러 리스너 객체
		...
		public void save() {
			...
			fileDir = saveOpen.getDirectory();// 사용자가 선택한 '디텍토리명' 얻어오기
	...
	class Buttons extends JPanel implements ActionListener{
		...
		public void nextData() {
			if(showTable.row < showTable.datamodel.getRowCount() -1) {
				showTable.row++;
				preBtn.setEnabled(true); // '추가' 버튼 비활성화
		...
		public void deleteData() {
			...
			if(yes_no_select == JOptionPane.YES_NO_CANCEL_OPTION) {
				addBtn.setEnabled(true);
	...
	class ShowTable extends MouseAdapter{
		...
		public void mouseClicked(MouseEvent e) {
			...
			//JTable에 있는 데이터를 west 객체의 input 객체 Jtextfield 객체 보여주기
			Info();
		...
		public void Info() {
			...
			west.output.label[0].setText("   성별 : " + "  " + showTable.table.getValueAt(row, 6));
			west.output.label[0].setText("   출생지역 : " + "  " + showTable.table.getValueAt(row, 7));
			west.output.label[0].setText("   생일 : " + "  " + showTable.table.getValueAt(row, 8));
